...
- [ ] **Step 2: Confirm exact package candidates**
...
- [ ] **Step 4: Verify installed tools and initial services**
...
**Files:**
- Create remotely: `/etc/mysql/mysql.conf.d/ari.cnf`
...
- [ ] **Step 1: Write the isolated MySQL network configuration**

```bash
ssh ubuntu@44.207.251.65 \
  'sudo tee /etc/mysql/mysql.conf.d/ari.cnf >/dev/null && sudo chown root:root /etc/mysql/mysql.conf.d/ari.cnf && sudo chmod 0644 /etc/mysql/mysql.conf.d/ari.cnf' <<'EOF'
[mysqld]
...
- [ ] **Step 2: Validate configuration before restart**
...
- [ ] **Step 3: Configure both root accounts without persisting the password**
...
- [ ] **Step 4: Restart and enable MySQL**
...
- [ ] **Step 5: Verify accounts and network binding**
...
- [ ] **Step 2: Create the virtual environment and install test/runtime dependencies**

```bash
ssh ubuntu@44.207.251.65 \
  'sudo -u ari python3 -m venv /opt/ari/.venv && sudo -u ari /opt/ari/.venv/bin/python -m pip install --upgrade pip && sudo -u ari /opt/ari/.venv/bin/python -m pip install "fastapi[standard]" pytest'
```

Expected: exit 0; FastAPI, Uvicorn, and pytest import under `/opt/ari/.venv/bin/python`.

- [ ] **Step 3: Write the health test before the application**
...
```bash
ssh ubuntu@44.207.251.65 \
  'cd /opt/ari/smoke && sudo -u ari /opt/ari/.venv/bin/python -m pytest -q'
```

Expected: FAIL during collection because module `app` does not exist.

- [ ] **Step 5: Write the minimal FastAPI application**
...
- [ ] **Step 6: Run the test and lock resolved versions**

```bash
ssh ubuntu@44.207.251.65 \
  'cd /opt/ari/smoke && sudo -u ari /opt/ari/.venv/bin/python -m pytest -q && sudo -u ari /opt/ari/.venv/bin/python -m pip freeze --all | sudo -u ari tee /opt/ari/smoke/requirements.lock >/dev/null'
```
...
- [ ] **Step 2: Validate the unit before enabling it**
...
- [ ] **Step 4: Verify systemd, loopback binding, and direct response**
...
- [ ] **Step 2: Enable the smoke site and release port 80**
...
- [ ] **Step 4: Verify listeners and local proxy response**
...
- [ ] **Step 1: Verify all remote services and listeners in one fresh check**

```bash
ssh ubuntu@44.207.251.65 \
  'set -eu; for service in mysql nginx ari-smoke; do systemctl is-active "$service"; systemctl is-enabled "$service"; done; mysql --version; nginx -v; /opt/ari/.venv/bin/python -c "import fastapi, uvicorn; print(fastapi.__version__, uvicorn.__version__)"; sudo ss -lntp; curl --fail --silent --show-error http://127.0.0.1:8000/health'
```

Expected: all services active/enabled, versions printed, required listeners present, and health JSON returned.

- [ ] **Step 2: Verify the health endpoint from the local workstation**
...
- [ ] **Step 3: Verify public MySQL TCP reachability**
...
- [ ] **Step 4: Verify `root@%` authentication from outside the server**
