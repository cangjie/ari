...
> **2026-09-26：本指引已全部执行完成，不要再照着跑一遍。** 评分由 Codex 完成（09-25），
> 审计、翻译、写库、导出由 Claude 完成（09-26），结果见 `PROGRESS.md` 与 `AGENTS.md` 第 16 条。
> 执行中改过两处，与下文不同：审计的空话校验改成了就地剔除；审计读简介改为按 `DESC_LANG` 取中文原文。
